...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased">
        ...
        {{-- Загружаем catalog-menu.js только на нужных страницах --}}
        @if(Request::routeIs('welcome') || Request::routeIs('catalog') || Request::routeIs('catalog.category'))
            <script src="{{ asset('js/catalog-menu.js') }}"></script>
        @endif

        {{-- Здесь будут скрипты, добавленные с помощью @push('scripts') --}}
        @stack('scripts')

        {{-- Скрипты Ion.RangeSlider и зависимость jQuery --}}
        {{-- jQuery нужен для Ion.RangeSlider --}}
        ...
        <div class="container">
            ...
            <div class="footer-actions-wrapper">
                <div class="footer-actions">
                    {{-- Кнопка Связаться --}}
                    <a href="{{ route('delivery') }}" class="btn btn-light footer-btn">
                        <span class="btn-emoji">👋</span> Доставка
                    </a>
                    {{-- Кнопка Отзывы --}}
                    <a href="{{ route('shops') }}" class="btn btn-light footer-btn">
